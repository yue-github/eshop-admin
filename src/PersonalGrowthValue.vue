<template scoped>
	<div>
		...
		<el-col :span="4"><bc-statbox title="总成长值" v-bind:part1="PersonalGrowthValueNum" back="#409EFF"></bc-statbox></el-col>
			...
			<el-col :span="24">
				<bc-panel title="查询结果" icon="icon-grid icons" style="position: relative;">
					...
					<el-table :data="table.items" border @selection-change="handleSelectionChange" style="width: 100%">
						...
						<el-table-column prop="amount" label="成长值">
						</el-table-column>
						...
						<!--<el-table-column prop="created_at" label="备注">
						</el-table-column>-->
	...
	module.exports = {
		...
		methods: {
			...
			handleSearch() {
				...
				var me = this;
				this.$get("admin/customer/getGrowth", key)
					.then2(function(response) {
						...
						for(var i =0;i<me.table.items.length;i++){
							...
							if(me.table.items[i].source==4){
                            me.table.items[i].source="注册"
                        }else if(me.table.items[i].source==6){
                            me.table.items[i].source="完善个人信息";
                        }else if(me.table.items[i].source==8){
                            me.table.items[i].source="邀请好友注册成功一次";
                        }else if(me.table.items[i].source==10){
                            me.table.items[i].source="每日签到";
                        }else if(me.table.items[i].source==12){
                            me.table.items[i].source="评论商品";
                        }else if(me.table.items[i].source==14){
                            me.table.items[i].source="点击广告";
                        }else if(me.table.items[i].source==36){
                            me.table.items[i].source="评论被管理员置顶前五";
                        }else if(me.table.items[i].source==16){
                            me.table.items[i].source="分享活动链接";
                        }else if(me.table.items[i].source==19){
                            me.table.items[i].source="购买特产类商品第三方支付";
                        }else if(me.table.items[i].source==22){
                            me.table.items[i].source="购买特产类商品钱包支付";
                        }else if(me.table.items[i].source==25){
                            me.table.items[i].source="购买旅游类产品第三方支付";
                        }else if(me.table.items[i].source==28){
                            me.table.items[i].source="购买旅游类产品钱包支付";
                        }else if(me.table.items[i].source==39){
							...
							me.PersonalGrowthValueNum = me.table.items[i].amount+me.PersonalGrowthValueNum;
						}
					}).catch(function(err) { 
						console.log(err);
					});
