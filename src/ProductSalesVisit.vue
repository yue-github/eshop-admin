<template scoped>
	<div>
		<!--查询弹出框-->
		<el-dialog title="查询产品销量" v-model="search.visible" size="small">
			<el-form :inline="true" :model="search" class="demo-form-inline" label-width="100px">
				<el-col :span="12">
					...
					<el-input placeholder="请输入名称关键字" v-model="search.data.customerName" :disabled="!search.useCustomerName" class="bottom20 input-size">
						<el-checkbox label="产品名称" slot="prepend" v-model="search.useCustomerName"></el-checkbox>
					</el-input>
				</el-col>
				<el-col :span="12">
					<el-row class="bottom20">
		...
		methods: {
			...
			handleSearch() {
				...
				if(this.search.useCustomerName && this.search.data.customerName) {
					...
					me.searchStr += ' 产品名称： ' + key.customerName;
					if(key.customerName != '') {
						me.searchStr += '；';
					}
				}
				...
				if(this.search.useCustomerName && this.search.data.customerName) {
					key.name = this.search.data.customerName;
				}
				if(this.search.useStartDatetime && this.search.data.startDatetime) {
				...
				this.$get("admin/salesVolume/productSalesVolume", key)
					.then2(function(response) {
