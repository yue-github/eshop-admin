<template scoped>
	<div>
		<!--查询弹出框-->
		<el-dialog title="查询通币" v-model="search.visible" size="small">
      ...
      <el-col :span="4"><bc-statbox title="总通币数" v-bind:part1="PersonalCurrencyNum" back="#409EFF"></bc-statbox></el-col>
    </el-row>
		<!-- <el-row :gutter="20" class="bottom20">
			...
			<el-col :span="24">
				<bc-panel title="查询结果" icon="icon-grid icons" style="position: relative;">
					...
					<el-table :data="table.items" border @selection-change="handleSelectionChange" style="width: 100%">
						...
						<el-table-column prop="amount" label="通币数量">
						</el-table-column>
						<el-table-column prop="source" label="通币来源">
						</el-table-column>
	...
	module.exports = {
		...
		methods: {
			...
			handleSearch() {
				...
				var me = this;
				this.$get("admin/customer/getGold", key)
					.then2(function(response) {
						...
						for(var i =0;i<me.table.items.length;i++){
							...
							if(me.table.items[i].source==18){
                            me.table.items[i].source="购买特产类商品第三方支付"
                        }else if(me.table.items[i].source==21){
                            me.table.items[i].source="购买特产类商品钱包支付";
                        }else if(me.table.items[i].source==24){
                            me.table.items[i].source="购买旅游类产品第三方支付";
                        }else if(me.table.items[i].source==27){
                        ...
                        me.PersonalCurrencyNum = me.table.items[i].amount+me.PersonalCurrencyNum;
						}
					}).catch(function(err) { 
						console.log(err);
					});
