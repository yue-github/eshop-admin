		...
		<el-dialog title="高级查询" v-model="search.visible" size="small">
			<el-form :inline="true" :model="search" class="demo-form-inline" label-width="100px">
				<el-col :span="12">
					...
					<el-input placeholder="请输入订单号关键字" v-model="search.data.orderCode" :disabled="!search.useOrderCode" class="bottom20 input-size">
						<el-checkbox label="订单号" slot="prepend" v-model="search.useOrderCode"></el-checkbox>
					</el-input>

					<el-input placeholder="请输入商铺名称关键字" v-model="search.data.name" :disabled="!search.useName" class="bottom20 input-size">
						<el-checkbox label="商铺名称" slot="prepend" v-model="search.useName"></el-checkbox>
					</el-input>
				...
				<el-col :span="12">
					<el-input placeholder="请输入产品名称关键字" v-model="search.data.productName" :disabled="!search.useProductName" class="bottom20 input-size">
						<el-checkbox label="产品名称" slot="prepend" v-model="search.useProductName"></el-checkbox>
					</el-input>

					<el-input placeholder="请输入审核人名称关键字" v-model="search.data.operator" :disabled="!search.useOperator" class="bottom20 input-size">
					...
					<el-tabs v-model="tabIndex" @tab-click="handleClick">
						<el-tab-pane label="未审核" name="0">
							<el-table :data="table.items" border @selection-change="handleSelectionChange" style="width: 100%">
								...
								<el-table-column prop="tradeCash" label="退款金额">
								</el-table-column>
								<el-table-column prop="refundCash" label="交易金额">
								</el-table-column>
								<el-table-column prop="isGeted" label="收货状态">
								</el-table-column>
						...
						<el-tab-pane label="通过" name="1">
							<el-table :data="table.items" border @selection-change="handleSelectionChange" style="width: 100%">
								...
								<el-table-column prop="remitTime" label="处理时间">
								</el-table-column>
								<el-table-column prop="productName" label="产品名称">
								</el-table-column>
								<el-table-column prop="tradeCash" label="退款金额">
						...
						<el-tab-pane label="已打款" name="3">
							<el-table :data="table.items" border @selection-change="handleSelectionChange" style="width: 100%">
								...
								<el-table-column prop="successTime" label="退款时间">
								</el-table-column>
								<el-table-column prop="productName" label="产品名称">
								</el-table-column>
								<el-table-column prop="tradeCash" label="退款金额">
					...
					<el-row slot="footer">
						<el-col :span="8">
							<el-button-group>
								...
								<el-button-group>
									<el-button type="info" size="small" @click="exportByOrder()"><i class="icon-arrow-down-circle icons"></i> 导出</el-button>
					...
					<el-form ref="form" :model="form.data" label-width="120px">
						...
						<el-row>
							<el-col :span="12">
								...
								<el-form-item label="订单状态" :label-width="formLabelWidth">
									<el-input v-model="form.data.order.status" :disabled="form.disabled"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="12">
								...
								<el-form-item label="下单时间">
									<el-input v-model="form.data.order.payTime" :disabled="form.disabled"></el-input>
								</el-form-item>
						...
						<template>
							<el-table :data="form.data.productOrder" style="width: 100%">
								...
								<el-table-column prop="totalActualProductPrice" label="单价">
								</el-table-column>
								<el-table-column prop="totalDeliveryCost" label="邮费">
								</el-table-column>
								<el-table-column prop="totalProductCost" label="成本">
								</el-table-column>
								<el-table-column prop="unitOrdered" label="数量">
								</el-table-column>
								<el-table-column prop="totalPrice" label="小计">
								</el-table-column>
								<el-table-column prop="status" label="明细状态">
								</el-table-column>
							</el-table>
						</template>
						...
						<el-form-item label="审核是否通过">
							<el-select v-model="form.data.status" style="width: 100%" @change="formChangeFun">
	...
	module.exports = {
		data: function() {
			return {
				...
				search: {
					...
					useEndTime: true,
					useProductName: true,
					useTradeCashMoreThan: true,
					useTradeCashLessThan: true,
					useRefundCashMoreThan: true,
					useRefundCashLessThan: true,
					useIsGeted: true,
					useOperator: true,
					data: {
						orderCode: '',
		...
		methods: {
			...
			handlCheckSubmit() {
				...
				this.$post('admin/back/check', data).then2(function(response) {
					me.$notify({
						title: '成功',
						message: '保存成功',
						type: 'success'
			...
			handleSubmit() {
				...
				me.$confirm('您现在选择的是打款功能, 是否继续?', '提示', {
					...
					this.$post('admin/back/audit', data).then2(function(response) {
			...
			handleEdit(id, row,type) {
				...
				this.$get('admin/back/get', {
						...
						switch(me.form.data.order.status){
							case 1: me.form.data.order.status = "待付款";break;
							case 2: me.form.data.order.status = "待发货";break;
							case 3: me.form.data.order.status = "待收货";break;
							case 4: me.form.data.order.status = "待评价";break;
							case 5: me.form.data.order.status = "已评价";break;
							case 6: me.form.data.order.status = "已取消";break;
							case 7: me.form.data.order.status = "订单完成";break;
						}
						if(type === 1){
			...
			getSearchKey(key) {
				...
				if(this.search.useEndTime && this.search.data.endTime && this.search.useTimeType && this.search.data.timeType) {
					...
					me.searchStr += '截止申请时间：' + key.endTime;
					if(key.endTime != '') {
						me.searchStr += '；';
					}
				}
			...
			handleSearch() {
				if(this.tabIndex == 0) {
					url = 'admin/back/many?status=0';
				} else if(this.tabIndex == 1) {
					url = 'admin/back/many?status=1';
				} else if(this.tabIndex == 2){
					url = 'admin/back/many?status=2';
				}else{
					url = 'admin/back/many?status=4';
				}
					...
					.then2(function(response) {
						...
						switch(me.table.items[i].status) {
								case 0:
									...
									break;
								case 1:
									me.table.items[i].status = "审核通过";
									break;
								case 2:
									me.table.items[i].status = "审核不通过";
									break;
								case 3:
									me.table.items[i].status = "已收货";
									break;
								case 4:
									me.table.items[i].status = "已退款";
									break;
							}
							...
							if(me.table.items[i].isGeted == false) {
								me.table.items[i].isGeted = "未收货";
							} else if(me.table.items[i].isGeted == true) {
								me.table.items[i].isGeted = "已收货";
							}
						}
					}).catch(function(err) { 
						console.log(err);
			...
			//导出Excel表
			exportByOrder: function(type) {
				if(type == 0) {
					url = 'admin/back/exportBack?status=0';
				} else if(type == 1) {
					url = 'admin/back/exportBack?status=1';
				} else if (type == 2) {
					url = 'admin/back/exportBack?status=2';
				} else {
					url = 'admin/back/exportBack?status=4';
				}
